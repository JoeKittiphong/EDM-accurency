...
// test function 
// call this funtion from input-ento.js 
const tryGetValue=()=>{
	columnA = document.querySelector("#columnA")
	columnB = document.querySelector("#columnB")
	columnAB = document.querySelector("#resultAB")
    let dataA = []
    let dataB = []
	let resultAB =[]
    for(let i in columnA.children){
		if(columnA.children[i].value){
			dataA.push(parseInt(columnA.children[i].value))
		}
    }
	for(let j in columnB.children){
		if(columnB.children[j].value){
			dataB.push(parseInt(columnB.children[j].value))
		}
	}
	for(let i in dataB){
		resultAB.push(dataA[i]+dataB[i])
		columnAB.children[i].value = Math.abs(dataA[i] - dataB[i])
	}
...
//  use this function for generate graph form 
initBloc("graph","grid","entoLine","line")